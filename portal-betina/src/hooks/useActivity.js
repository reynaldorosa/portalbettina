...
export const useActivity = (activityId, options = {}) => {
  ...
  const startGame = useCallback(async () => {
    try {
      ...
      if (metrics && userId) {
        ...
        componentLogger.current.debug('Sessão de métricas iniciada', { sessionId: sessionIdRef.current });
      }
  ...
  // Registrar sucesso
  const recordSuccess = useCallback((data = {}) => {
  ...
  }, [metrics, progress, sound, accessibilitySettings]);

  // Registrar erro
  const recordError = useCallback((data = {}) => {
    setSessionMetrics(prev => {
  ...
  const showFeedback = useCallback((message, type = 'info', duration = 3000) => {
