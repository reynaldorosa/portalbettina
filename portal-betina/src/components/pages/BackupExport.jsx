...
const Alert = styled.div`
  background: ${props => 
    props.type === 'success' ? 'var(--primary-green-light)' :
    props.type === 'error' ? 'var(--primary-red-light)' : 
    'var(--primary-blue-light)'
  };
  color: ${props => 
    props.type === 'success' ? 'var(--primary-green)' :
    props.type === 'error' ? 'var(--primary-red)' : 
    'var(--primary-blue)'
  };
...
const BackupExport = () => {
    ...
    try {
      ...
      if (isDbConnected) {
        ...
        if (importPreview.data.accessibilitySettings) {
          localStorage.setItem('betina_accessibility_settings', 
            JSON.stringify(importPreview.data.accessibilitySettings));
        }
        
        if (importPreview.data.preferences) {
          localStorage.setItem('betina_user_preferences', 
  ...
  return (
    <BackupContainer>
      <SectionTitle>
        <span role="img" aria-label="backup">💾</span> Backup e Exportação de Dados
      </SectionTitle>
      ...
      >
        ...
        <CardDescription>
          Crie um backup de seus dados para manter seus progressos seguros ou transferir para outro dispositivo.
        </CardDescription>
        ...
        <CheckboxContainer>
          <CheckboxLabel>
            <Checkbox 
              ...
              onChange={(e) => setExportOptions({...exportOptions, userProfiles: e.target.checked})}
            />
            Perfis de Usuário
          </CheckboxLabel>
          ...
          <CheckboxLabel>
            <Checkbox 
              ...
              onChange={(e) => setExportOptions({...exportOptions, gameProgress: e.target.checked})}
            />
            Progresso dos Jogos
          </CheckboxLabel>
          ...
          <CheckboxLabel>
            <Checkbox 
              ...
              onChange={(e) => setExportOptions({...exportOptions, accessibilitySettings: e.target.checked})}
            />
          ...
          <CheckboxLabel>
            <Checkbox 
              ...
              onChange={(e) => setExportOptions({...exportOptions, preferences: e.target.checked})}
            />
            Preferências Gerais
          </CheckboxLabel>
        </CheckboxContainer>
        ...
        {backupData && (
          <>
            ...
            <Button 
              ...
              whileTap={{ scale: 0.95 }}
            >
              <span role="img" aria-hidden="true">⬇️</span> Baixar Arquivo de Backup
            </Button>
          </>
        )}
      </Card>
      ...
      >
        ...
        <FileLabel htmlFor="backup-file">
          <span role="img" aria-hidden="true">📂</span> Selecionar Arquivo de Backup
          <FileInput 
            type="file" 
            id="backup-file" 
            accept=".json" 
        ...
        {importFile && (
          <p>Arquivo selecionado: {importFile.name}</p>
        )}
      ...
      >
        ...
        <Alert type="info">
          <p><strong>Status da Sincronização</strong>: {isDbConnected ? 'Ativada' : 'Desativada'}</p>
          <p>{isDbConnected ? 
