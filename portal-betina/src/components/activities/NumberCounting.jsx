...
function NumberCounting({ onBack }) {
  ...
  const startGame = async () => {
    ...
    autoSpeak("Jogo de contagem iniciado! Conte os objetos na tela e escolha o número correto.", 1000)
  }
  ...
  const generateNewRound = () => {
    ...
    while (options.length < adaptedOptions) {
      ...
      if (Math.random() < 0.7) {
        ...
        wrongNumber = count + (Math.random() < 0.5 ? 1 : -1) * 
    ...
    setNumberOptions(options.sort(() => Math.random() - 0.5).map(num => ({
      number: num,
      isCorrect: num === count
    })))
  ...
  const handleObjectClick = (objectId) => {
    ...
    if (newCountedObjects.length === correctCount) {
      setTimeout(() => {
        ...
        speakFeedback(`${message} Você contou ${correctCount} objetos corretamente! Ganhou ${score} pontos!`, true)
  ...
  const handleNumberSelect = (option) => {
    ...
    <GameContainer>
      ...
      <ActivityTitleSection>
        ...
        <ActivitySubtitle>
          Aprenda a contar objetos de forma divertida
        </ActivitySubtitle>
      </ActivityTitleSection>
      ...
      {objects.length > 0 && (
        <CountingArea>
          <QuestionDisplay>
            {gameMode === 'count' 
              ? `Conte os objetos clicando neles:` 
              : `Quantos objetos há aqui?`
            }
          </QuestionDisplay>
      ...
      <ControlButtons>
        {!gameStarted ? (
          ...
          <>
            ...
            <ActionButton
              className="audio"
              onClick={() => speakNumberCustom(correctCount)}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              🔊 Ouvir Resposta
