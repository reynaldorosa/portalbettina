...
export class MultisensoryMetricsService {
  ...
  async initializeOrchestrator() {
    try {
      // Aguardar o SystemOrchestrator estar disponível
      if (typeof getSystemOrchestrator === 'function') {
        this.orchestratorRef = getSystemOrchestrator()
        logger.info('🎯 SystemOrchestrator integrado com MultisensoryMetricsService')
      } else {
        logger.warn('⚠️ SystemOrchestrator não disponível ainda, tentando novamente...')
        // Tentar novamente após um delay
        setTimeout(() => this.initializeOrchestrator(), 1000)
      }
    } catch (error) {
