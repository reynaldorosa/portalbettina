...
import { createAdaptiveModel, analyzeLearningSessions } from './adaptiveML'
...
export const generateProgressReport = async (userId = null) => {
  ...
  try {
    ...
    for (const gameId of games) {
      ...
      if (sessions && sessions.length > 0) {
        ...
        const gameScore = sessions.reduce((sum, s) => sum + s.score, 0)
  ...
  const trends = Object.values(report.gameReports).map(g => g.trend).filter(t => t !== null)
  if (trends.length > 0) {
...
export const generateSuggestions = (report) => {
  ...
  if (report.overallProgress.learningTrend > 0.05) {
    suggestions.push({
      type: 'general',
      message: 'Seu progresso está ótimo! Continue jogando regularmente para desenvolver suas habilidades.'
    })
  } else if (report.overallProgress.learningTrend < -0.05) {
    suggestions.push({
      type: 'general',
      message: 'Tente jogar por períodos mais curtos, mas com mais frequência para melhorar seu aprendizado.'
    })
  }
    ...
    for (const gameId of gameIds) {
      ...
      if (gameData) {
        ...
        if (sessions.length > 0) {
          ...
          const gameAccuracy = sessions.reduce((sum, session) => sum + (session.accuracy || 0), 0) / sessions.length
