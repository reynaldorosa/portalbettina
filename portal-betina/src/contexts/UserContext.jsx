...
// Key para armazenar o ID do usuário no localStorage
...
export function UserProvider({ children }) {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        const storedId = localStorage.getItem(USER_ID_KEY);        if (storedId) {
  ...
  const recordPerformance = async (gameId, performanceData) => {
    ...
    try {
      ...
      const adaptiveParams = await databaseService.getAdaptiveParameters(
        gameId, 
        difficultyLevel
      );
