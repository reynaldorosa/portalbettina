...
class DatabaseService {
  ...
  async getAdaptiveParameters(gameId, difficulty) {
    ...
    // Se está offline ou não tem parâmetros, usar padrão
    if (this.isOfflineMode) {
    ...
    if (!apiAvailable) {
      this.isOfflineMode = true;
      return this.getAdaptiveParameters(gameId, difficulty);
    }
    ...
    try {
      ...
      console.error('Erro ao buscar parâmetros adaptativos, usando offline:', error);
      this.isOfflineMode = true;
      return this.getAdaptiveParameters(gameId, difficulty);
    }
  }
